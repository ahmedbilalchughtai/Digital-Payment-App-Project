...
class signupform extends Component {
  ...
  handleRegister() {
    if (this.state.activeTab == "Pasien") {
      ...
      // return;
      if (
        this.state.selectedDokter == "key0" ||
        this.state.selectedDokter == undefined
      ) {
  ...
  _renderFormReg() {
    let dataDokter = [{ nama: "Pilih Dokter", value: "key0" }];
    if (this.state.dokter.length > 0) {
      this.state.dokter.forEach((data) =>
        dataDokter.push({ nama: data.nama, id_dokter: data.id_dokter })
      );
    }
    let comboDokter = dataDokter.map((data, index) => {
      return <Item key={index} label={data.nama} value={data.id_dokter} />;
    });
    ...
    return (
      <Form style={styles.formLogin}>
        ...
        <Picker
          style={styles.st_kelamin}
          mode="dropdown"
          selectedValue={this.state.selectedGender}
          onValueChange={this.onValueGender.bind(this)}
        >
          <Item label="Jenis Kelamin" value="key0" />
          <Item label="Laki-Laki" value="l" />
          <Item label="Wanita" value="p" />
        </Picker>
        <Text style={styles.st_inputfnt2}>Dokter :</Text>
        <Picker
          style={styles.st_dokter}
          mode="dropdown"
          selectedValue={this.state.selectedDokter}
          onValueChange={this.onValueDokter.bind(this)}
        >
          {comboDokter}
        </Picker>
        <DatePicker
